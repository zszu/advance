...
JS;
    ...
    {
        return Yii::$app->params['uploadConfig'];
    }
    ...
    {
        $src = Yii::$app->params['uploadConfig']['defaultImage'];
        $eles = [];
        if ($value = $model->$attribute) {
            $protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
            $domain = Yii::$app->params['uploadConfig']['domain'] ?? $protocol . $_SERVER['HTTP_HOST'];

            $src = $this->_validateUrl($value) ? $value : $domain . $value;
        }
    ...
    public function renderMultiImage ($model, $attribute)
    {
        ...
        if ($srcTmp) {
            is_string($srcTmp) && $srcTmp = explode($config['delimiter'], $srcTmp);
            $inputName = Html::getInputName($model, $attribute);
            foreach ($srcTmp as $k => $v) {
                $dv = $this->_validateUrl($v) ? $v : $config['domain'] . $v;
                $src = $v ? $dv : $config['baseConfig']['defaultImage'];
                $eles = [];
                $eles[] = Html::img($src, ['class' => 'img-responsive img-thumbnail cus-img']);
